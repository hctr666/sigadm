    ...
    switch ($accion) {
        ...
        case 'grabar_editar':
            ...
            if ($contratoDAO->editar($contrato) == 1) {
                ...
                echo "<script>alert('No se edit®Æ el contrato :/');</script>";
            }
        ...
        case 'grabar_nuevo':
            ...
            if ($contratoDAO->agregar($contrato) == 1) {
                ...
                echo "<script>alert('No se agregÛ el cotrato :(');</script>";               
            }
            ...
            if ($contrato->codi_cond == '09' || $contrato->codi_cond == '10') {
                ...
                if ($detallePracdao->agregarPracticante($detallePrac) == 1) {
                    #echo "<script>alert('se agregÛ el practicante');</script>";
                    $criterioBuscar = substr($_GET['nombre'], 0, 1);
                    #$listacontratos = $contratoDAO->seleccionarReporte($contrato, $criterioBuscar);
                } else {
                    echo "<script>alert('No se agregÛ nada :(');</script>";
                }
            }
            $listacontratos = $contratoDAO->seleccionarReporte($contrato, $criterioBuscar);
            require_once '../contratos.php';
        ...
        case 'imprimirContrato':
            ...
            switch ($registro->codi_cond) {
                ...
                case '05':
                    $aÅ0ä9o_tmp = substr($registro->fech_inic, 0,4);
                    #$aÅ0ä9oactual = substr(date("Y-m-d"), 0,4);
                    #echo "<script>alert($aÅ0ä9o_tmp)</script>";
                    $tmp_alta = $contratoDAO->cargarFechaTmpAlta($aÅ0ä9o_tmp);
