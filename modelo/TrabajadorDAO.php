...
class TrabajadorDAO {
    ...
    function listar($codi_empr) {
        $sql = "select * from trabajador where codi_empr='$codi_empr' order by appa_trab,apma_trab,nomb_trab  ";
    ...
    function listarCumpleaniosMes(){
        $sql="select nume_dni,appa_trab ,apma_trab, nomb_trab, DATE_FORMAT(fech_naci,'%d') as dia from trabajador where date_format (fech_naci, '%m') = date_format (now(), '%m') order by DATE_FORMAT(fech_naci,'%d/%m')";
        $lista = MySQL::arrayObject($sql);
        return $lista;
    }
    ...
    function listar_todo($codi_empr) {
        $sql = "SELECT t.*,desc_dist FROM trabajador t inner join distrito d on t.codi_dist=d.codi_dist where t.codi_empr='$codi_empr' order by appa_trab,apma_trab,nomb_trab";
        $lista = MySQL::arrayObject($sql);
        return $lista;
    }

    function seleccionar($codi_empr, $codi_trab) {
        $sql = "SELECT * FROM trabajador WHERE codi_trab='" . $codi_trab . "' and codi_empr='$codi_empr' order by appa_trab,apma_trab,nomb_trab ";
    ...
    function agregar(Trabajador $trabajador) {
        ...
        $sql = "INSERT INTO trabajador(codi_empr,codi_trab,codi_sap,nume_dni,
                              ...
                              VALUES('$trabajador->codi_empr',$correlativo,'$trabajador->codi_sap','$trabajador->nume_dni',
                                     ...
                                     '$trabajador->dire_trab','$trabajador->codi_dist')";
    ...
    function editar(Trabajador $trabajador) {
        $sql = "update trabajador set codi_sap='$trabajador->codi_sap',
       								 nume_dni='$trabajador->nume_dni',
                                                                 appa_trab='$trabajador->appa_trab',
                                                                 apma_trab='$trabajador->apma_trab',
       								 nomb_trab='$trabajador->nomb_trab',
       								 fech_naci='$trabajador->fech_naci',
       								 dire_trab='$trabajador->dire_trab', 
                                                                 codi_dist='$trabajador->codi_dist' 
